 ...
 */
class NetworkBadRespondedException(val statusCode: Int, val resStr: String, val reqUrl: String? = null): Exception("Request " + (if (reqUrl != null) "to ${reqUrl} " else "") + "responded with status ${statusCode} : " + resStr), NetworkResInterface {}
...
class NetworkResponse <T>(
    /** 返回码 */val statusCode: Int,
    /** 原始的未经解析的返回字符串 */val resStr: String,
    /** 返回的头 */val headers: Map<String, String>,
    /** 经过JSON解析为T类后的对象 */clazz: Class<T>? = null): NetworkResInterface
{
    val resObj: T?
    init{
 ...
 *
 * runBlocking{ request(...).await() }//阻塞主线程，直到得到结果。
 *
 * 或GlobalScope.launch{ request(...).await() }//异步处理，所在函数会立刻返回。
 *
 ...
 */
suspend fun <T> request(url: String, method: RequestMethod = RequestMethod.GET, body: Any? = null, responseClass: Class<T>? = null, query: Map<String, String>? = null, headers: Map<String, String>? = null, timeOut: Int = 5000): NetworkResponse<T> {
    ...
    val resObj = withContext(Dispatchers.IO) {
        try {
            val client =
                HttpClient.newBuilder().followRedirects(HttpClient.Redirect.ALWAYS).version(HttpClient.Version.HTTP_1_1).build()!!
 ...
 * @throws NetworkBadRespondedException 如果网络连接失败或服务器返回4XX/5XX
 */
suspend fun <T> requestUploadFile(url: String, method: RequestMethod = RequestMethod.POST, file: File, responseClass: Class<T>? = null, query: Map<String, String>? = null, headers: Map<String, String>? = null, timeOut: Int = 5000): NetworkResponse<T> {
    var realUrl: String = url
    if(url.contains('?')){
        if(query != null)throw Exception("不能同时在url中包括?和对query传入非null值！")
    }else{
    ...
    val resObj = withContext(Dispatchers.IO) {
        try {
            ...
            when (method) {
                RequestMethod.POST -> {
                    requestBuilder.POST(HttpRequest.BodyPublishers.ofFile(Path.of(file.path)))
                }
                else ->{
                    throw NotImplementedError("不支持此方法")
                }
 ...
 * @throws NetworkBadRespondedException 如果网络连接失败或服务器返回4XX/5XX
 */
suspend fun requestDownloadFile(url: String, method: RequestMethod = RequestMethod.GET, file: File, body: Any? = null, query: Map<String, String>? = null, headers: Map<String, String>? = null, timeOut: Int = 5000): File {
    var realUrl: String = url
    if(url.contains('?')){
        if(query != null)throw Exception("不能同时在url中包括?和对query传入非null值！")
    }else{
